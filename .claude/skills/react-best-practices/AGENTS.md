...
> generating, or refactoring React codebases (Vite SPA). Humans  
> may also find it useful, but guidance here is optimized for automation  
> and consistency by AI-assisted workflows.
>
> This repo uses React 18, Vite, React Router, TanStack Query, Supabase, and SSR/prerendered pages on Netlify.

---

## Abstract

Comprehensive performance optimization guide for the hrkit frontend stack. The rules are prioritized by impact and intentionally avoid guidance that depends on newer React APIs or extra libraries not present in this repo.
...
1. [Eliminating Waterfalls](#1-eliminating-waterfalls) - **CRITICAL**
    ...
    - 1.3 [Prevent Waterfall Chains in Async Route Handlers](#13-prevent-waterfall-chains-in-async-route-handlers)
...
5. [Rendering Performance](#5-rendering-performance) - **MEDIUM**
    ...
    - 5.7 [Preserve Expensive Subtrees During Show/Hide](#57-preserve-expensive-subtrees-during-showhide)
...
This pattern works well in React Router loaders, Supabase edge-function handlers, and async utilities that have partial dependencies.

### 1.3 Prevent Waterfall Chains in Async Route Handlers
...
### 3.3 Use TanStack Query for Automatic Deduplication
...
In SSR/SSG frameworks, some values are intentionally different on server vs client (random IDs, dates, locale/timezone formatting). For these _expected_ mismatches, wrap the dynamic text in an element with `suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide real bugs. Don’t overuse it.
...
### 5.7 Preserve Expensive Subtrees During Show/Hide
...
function Dropdown({ isOpen }: Props) {
    return (
        <div hidden={!isOpen} aria-hidden={!isOpen} className={isOpen ? "block" : "hidden"}>
            <ExpensiveMenu />
        </div>
    );
}
...
- **Focused deferral**: Only the expensive render update is deprioritized

- **React 18-safe**: Avoids relying on async work inside the transition callback
...
If you need the same pattern in multiple places, extract the ref-sync logic into a local `useLatest` helper and keep the subscription effect dependent only on stable values like `event`.
...
3. [https://reactrouter.com](https://reactrouter.com)
